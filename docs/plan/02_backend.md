...
// LoginResponse
{ accessToken: string; refreshToken: string; user: { id, name, role } }
```
...
function recommendProduct(customer, products, interactions): ProductRecommendation {
  ...
  if (!hasProduct(products, 'SAVING')) {
    return { product: 'SHB Saving Account', confidence: 0.70 }
  }
  // ...more rules
}
```
...
```json
{
  ...
  "conversionRate": 0.24,
  "funnel": {
    "NEW": 30, "QUALIFIED": 25, "CONTACTED": 40,
    "PROPOSAL": 20, "NEGOTIATION": 15, "WON": 10, "LOST": 5
  },
